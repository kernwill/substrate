...
func runCollect(args []string, stdout, stderr io.Writer) int {
	...
	// S3, IAM, CloudTrail, and (if configured) the four Okta collectors
	...
	warning, err := writeCollectOutput(*out, s3Graph, iamGraph, cloudTrailGraph, okta)
	if err != nil {
		fmt.Fprintf(stderr, "substrate collect: %v\n", err)
		return 2
	}
	...
	summary := fmt.Sprintf("collected %d s3 bucket(s), %d iam user(s), and %d cloudtrail trail(s)",
		len(s3Graph.Buckets), len(iamGraph.Users), len(cloudTrailGraph.Trails))
	if okta != nil {
...
// writeCollectOutput writes s3Graph, iamGraph, and cloudTrailGraph to
// their AWS artifact files (always), and okta's four graphs to their own
...
func writeCollectOutput(out string, s3Graph *awscollectors.S3Graph, iamGraph *awscollectors.IAMGraph, cloudTrailGraph *awscollectors.CloudTrailGraph, okta *oktaResults) (warning string, err error) {
	tmpOut := out + ".tmp"
