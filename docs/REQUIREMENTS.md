...
8. 2026-09-18: live-AWS leg of the self-test (`substrate collect` against real S3/IAM resources, `/Users/willkern/substrate-self-test/SUBSTRATE-TESTING.md` step 5) is shelved, not done. Blocked on AWS account access: the account reached via the founder's existing sign-in sits inside an AWS Organization with a service control policy denying `s3:CreateBucket`, not controllable from that member account; a fresh-account signup attempt then hit a hard AWS Builder ID signup error requiring AWS Support (Request ID `a8e7428a-19c5-4985-bc38-7b481665c016`, 2026-09-18T16:31:30Z). Does not block Phase 1/2 self-testing overall - the static pass (compile, `ir query`, baseline, gate, the regression-remediate-reverify loop) needs no AWS account and is done and verified. Revisit once a clean AWS account is available (either AWS Support resolves the Builder ID issue, or a different device/network succeeds at fresh signup).
9. 2026-09-18: `docs/adr/0013`'s `not_satisfied` propagation rule - one predicate failure overrides an entire multi-control indicator's status, regardless of how many of that indicator's other referenced controls remain completely uncollected. Deliberate, not a bug (see the ADR's own reasoning: waiting for full coverage before ever surfacing a proven violation would sit on real findings indefinitely), but flagged for revisit once `gate`/PR-comment rendering (`docs/adr/0011`) has real `not_satisfied` output to work with: a multi-control indicator like `KSI-IAM-ELP` (34 referenced controls) flipping to `not_satisfied` over one bad resource could read, in a PR comment, like a much bigger regression than "one bucket." Revisit the messaging, not the underlying logic, once this is visible in a real run.
10. 2026-09-21: Okta `TokenSource` (`internal/frontend/collectors/okta/auth.go`) doesn't implement DPoP (Demonstrating Proof of Possession) token binding - a fresh Okta API Services app enforces it by default, so today's setup instructions (`docs/okta-api-scopes.md`, `substrate-self-test/SUBSTRATE-TESTING.md` §6) tell the customer to disable it on the app's General tab rather than the client supporting it. Okta is pushing DPoP as the default posture going forward, so this is a real gap to close, not a permanent workaround - implement DPoP support in `TokenSource` when there's time for it.
11. 2026-09-21: FR-3's "findings" halves are deferred across four collectors, all enablement-only today: GuardDuty (`internal/frontend/collectors/aws/guardduty.go`), AWS Config (`awsconfig.go` - also missing `RecordingGroup` scope detail, i.e. whether the recorder covers all resource types or a narrow subset), and Security Hub (`securityhub.go`) all collect subscription/enablement status only, not the actual findings each service produces. KMS (`kms.go`) collects rotation status only, not key policy documents (a real JSON policy needing actual policy-evaluation logic to be meaningful, not just a status flag). Security groups (`securitygroups.go`) collects IPv4 CIDR-based rules only, not IPv6 ranges or cross-security-group references (`UserIdGroupPairs`). Each is disclosed in its own file's doc comment and in `docs/adr/0008`'s per-collector addenda; this entry exists so the aggregate backlog is visible in one place without re-deriving it from nine separate ADR sections.
12. 2026-09-21: `cmd/substrate/collect.go` runs AWS collection unconditionally but gates Okta collection on `--okta-org-url` being given - a deliberate, disclosed asymmetry (no real customer has an Okta API Services app configured yet, and Okta has no ambient default credential chain the way AWS does), not an oversight. Revisit making Okta mandatory-by-default once Okta collection has real customer usage to justify that default.
13. 2026-09-21: The hand-rolled Okta REST client (`internal/frontend/collectors/okta/client.go`) does not follow Link-header pagination on `ListPolicies`, `ListSystemLogEvents`, or `ListUsers` past Okta's default page size - disclosed in the client's own comments, not yet exercised by any fixture or self-test/live org with enough data to hit the page boundary. Revisit once a real org's data volume makes this a live risk, or proactively before onboarding a customer with a large Okta org.
